...
export async function GET(
  ...
  try {
    ...
    const weekAgoInMelbourne = subDays(nowInMelbourne, 7)

    // Calculate start and end of the current week in Melbourne time
    const startOfCurrentWeek = startOfWeek(weekAgoInMelbourne, { weekStartsOn: 1 }) // 1 means week starts on Monday
    const endOfCurrentWeek = endOfWeek(weekAgoInMelbourne, { weekStartsOn: 1 })
